...
def register_error_handlers(app):
    ...
    def handle_unique_violation(e):
        return jsonify({
            'error': 'Conflict',
            'message': 'El registro ya existe, clave única duplicada (como cédula o correo).',
            'status': 409
        }), 409
    ...
    def handle_foreign_key_violation(e):
        return jsonify({
            'error': 'Bad Request',
            'message': 'La operación no puede realizarse porque el registro está relacionado con otros datos.',
            'status': 400
        }), 400
